...
@section('title', 'Edit Mahasiswa: ' . $mahasiswa->name)
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-1">Edit Mahasiswa</h1>
                    <p class="text-gray-600 text-sm">Perbarui informasi mahasiswa <span class="font-semibold text-gray-900">{{ $mahasiswa->name }}</span></p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <h2 class="text-base font-semibold text-gray-900">Edit Informasi Mahasiswa</h2>
                        <p class="text-gray-500 text-xs mt-0.5">Data yang wajib diisi ditandai dengan tanda bintang (*)</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('admin.mahasiswa.update', $mahasiswa->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    {{-- FOTO SECTION --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Foto Profil (Ukuran 4x6)</label>
                        ...
                        <div class="mb-3 flex justify-center">
                            <div id="previewContainer" class="relative border-2 {{ $mahasiswa->photo ? 'border-solid' : 'border-dashed' }} border-gray-300 rounded-lg overflow-hidden bg-gray-50 flex items-center justify-center shadow-inner" 
                                 style="width: 150px; height: 225px;">
                                
                                @if($mahasiswa->photo)
                                    <img id="photoPreview" src="{{ asset('storage/photos/' . $mahasiswa->photo) }}" class="w-full h-full object-cover">
                                    <div id="placeholderText" class="text-center p-4 hidden">
                                @else
                        ...
                        <button type="button" id="removePhoto" class="mt-2 text-xs text-red-600 font-bold {{ $mahasiswa->photo ? '' : 'hidden' }} hover:underline">
                            × RESET/HAPUS FOTO
                        </button>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nim" id="nim" value="{{ old('nim', $mahasiswa->nim) }}"
                                   class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('nim') border-red-500 @enderror" required>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $mahasiswa->name) }}"
                                   class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $mahasiswa->email) }}"
                               class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('email') border-red-500 @enderror" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_prodi" id="id_prodi" 
                                class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 @error('id_prodi') border-red-500 @enderror" required>
                            <option value="">-- Pilih Program Studi --</option>
                            @foreach($program_studi as $prodi)
                                <option value="{{ $prodi->id }}" {{ old('id_prodi', $mahasiswa->id_prodi) == $prodi->id ? 'selected' : '' }}>
                                    {{ $prodi->program_studi_id }} - {{ $prodi->nama_prodi }}
                                </option>
                            @endforeach
                        </select>
                    </div>
        ...
        {{-- INFO BOX --}}
        <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-xs text-yellow-800">
            <div class="flex">
                ...
                <ul class="list-disc list-inside space-y-1">
                    <li>Jika foto tidak diubah, sistem akan tetap menyimpan foto lama.</li>
                    <li>Pastikan ukuran file foto tidak melebihi 2MB.</li>
                </ul>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Simpan URL foto asli (jika ada) untuk fitur Reset
    const originalSrc = photoPreview.src;
    const hasOriginalPhoto = @json($mahasiswa->photo ? true : false);
    ...
    removeBtn.addEventListener('click', function () {
        photoInput.value = '';
        if (hasOriginalPhoto) {
            // Jika ada foto lama, kembalikan ke foto lama
            ...
            // Jika tidak ada foto dari awal, sembunyikan semua
            photoPreview.src = '';
